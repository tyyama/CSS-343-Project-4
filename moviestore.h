...
class MovieStore{
	...
	private:
		...
		stack<ClassicMovie*> destCMovies;
		stack<Customer*> destCustomers;
		...
		struct cMovieNode{//to handle collisions, the movie table will store a linked list in each element
			ClassicMovie *movie;//movie 
			cMovieNode *next=NULL;//next movie if a collision occurs 		
		};
		
		CustomerNode *customerTable[defaultSize];//table of customers
		MovieNode *movieTable[defaultSize];//table of movies
		cMovieNode *cMovieTable[defaultSize];//table of movies
		
		static int hashMovie(char,int);
		static int hashMovie(char,int,int);
		static int hashCustomer(int);
